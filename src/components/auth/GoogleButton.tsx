import { useGoogleLogin } from '@react-oauth/google';
import { useNavigate } from 'react-router-dom';
import { useAuth } from '../../context/AuthContext';
import { useToast } from '../../context/ToastContext';
import { getUsers, saveUser } from '../../utils/storage';
import s from '../../styles/form.module.css';

interface Props {
  label: string;
}

export function GoogleButton({ label }: Props) {
  const { login } = useAuth();
  const { showToast } = useToast();
  const navigate = useNavigate();
  const isConfigured = import.meta.env.VITE_GOOGLE_CLIENT_ID !== 'YOUR_GOOGLE_CLIENT_ID_HERE';

  const googleLogin = useGoogleLogin({
    onSuccess: async tokenResponse => {
      try {
        const info = await fetch('https://www.googleapis.com/oauth2/v3/userinfo', {
          headers: { Authorization: `Bearer ${tokenResponse.access_token}` },
        }).then(r => r.json()) as { email: string; name: string; picture?: string };

        const email = info.email.toLowerCase();
        const name = info.name || email.split('@')[0];
        const users = getUsers();
        if (!users[email]) {
          saveUser(email, { name, email, method: 'google', joined: Date.now() });
        }
        login({ name, email, method: 'google', picture: info.picture });
        showToast('Signed in with Google!');
        navigate('/welcome');
      } catch {
        showToast('Google sign-in failed. Try again.', 'error');
      }
    },
    onError: () => showToast('Google sign-in failed. Try again.', 'error'),
  });

  function handleClick() {
    if (!isConfigured) {
      showToast('Add VITE_GOOGLE_CLIENT_ID to .env to enable Google OAuth', 'error');
      return;
    }
    googleLogin();
  }

  return (
    <button type="button" className={s.googleBtn} onClick={handleClick}>
      <GoogleIcon />
      {label}
    </button>
  );
}

function GoogleIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" aria-hidden="true">
      <path fill="#4285F4" d="M17.64 9.2c0-.637-.057-1.251-.164-1.84H9v3.481h4.844a4.14 4.14 0 0 1-1.796 2.716v2.259h2.908c1.702-1.567 2.684-3.875 2.684-6.615z" />
      <path fill="#34A853" d="M9 18c2.43 0 4.467-.806 5.956-2.18l-2.908-2.259c-.806.54-1.837.86-3.048.86-2.344 0-4.328-1.584-5.036-3.711H.957v2.332A8.997 8.997 0 0 0 9 18z" />
      <path fill="#FBBC05" d="M3.964 10.71A5.41 5.41 0 0 1 3.682 9c0-.593.102-1.17.282-1.71V4.958H.957A8.996 8.996 0 0 0 0 9c0 1.452.348 2.827.957 4.042l3.007-2.332z" />
      <path fill="#EA4335" d="M9 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.463.891 11.426 0 9 0A8.997 8.997 0 0 0 .957 4.958L3.964 6.29C4.672 4.163 6.656 3.58 9 3.58z" />
    </svg>
  );
}
